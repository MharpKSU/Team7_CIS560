...
app.post('/api/deact-laptop', async(req, res) =>
{
    const{laptopId, laptopMake, laptopModel, dateDeactivated} = req.body;
    try{
        await sql.connect(dbConfig);
        const request = new sql.Request();
        ...
        request.input('dDeactivated', sql.DateTime, dateDeactivated);
